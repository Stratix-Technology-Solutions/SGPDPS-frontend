  ...
  return (
    <>
      ...
      {!isLoading && !data?.length && (
        <p className="text-neutral-medium/70 text-sm">No hay experiencias académicas registradas.</p>
      )}
        ...
        {data?.map((item) => (
          <li key={item.id}>
            ...
            >
              ...
              <p className="text-sm text-neutral-medium/70">{item.institution} · {getAcademicTypeLabel(item.type)}</p>
            </button>
          </li>
        ))}
      </ul>
